      ...
      >
        <div className="deznav-scroll mm-active d-flex flex-column justify-content-between ">
          <ul className="metismenu mm-show flex-1" id="menu">
            <li
              className={`mm-active hideDarrow ${!showAsideBar ? "hide" : ""}`}
              onClick={() => menuClicked("2363")}
            >
              <Link className="has-arrow" aria-expanded={parentId === "2363"}>
              ...
              <ul
                aria-expanded={parentId === "2363"}
                className={`mm-collapse left ${parentId === "2363" ? "mm-show" : ""}`}
              >
                <li
                  className={`mm-active sidebar-content ${childId === "dashboardTravels" ? "active" : ""}`}
                  onClick={(e) => subMenuClicked(e, "dashboardTravels")}
                >
                  <Link to="/viaggio/dashboard" className={`${childId === "dashboardTravels" ? "mm-active" : ""}`}>
                    {t("Dashboard Travels")}
                  </Link>
                </li>
                <li
                  className={`mm-active sidebar-content ${childId === "dashboardFintech" ? "active" : ""}`}
                  onClick={(e) => subMenuClicked(e, "dashboardFintech")}
                >
                  <Link to="fintech/admin" className={`${childId === "dashboardFintech" ? "mm-active" : ""}`}>
                    {t("Dashboard Fintech")}
                  </Link>
                </li>
                <li
                  className={`mm-active sidebar-content ${childId === "dashboardCRM" ? "active" : ""}`}
                  onClick={(e) => subMenuClicked(e, "dashboardCRM")}
                >
                  <Link to="/dashboard/crm" className={`${childId === "dashboardCRM" ? "mm-active" : ""}`}>
                    {t("Dashboard CRM")}
                  </Link>
                </li>
              </ul>
